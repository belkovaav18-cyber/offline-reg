...
ACCOUNTING_SHEET_NAME = 'Бухгалтерия'  # Новый лист для бухгалтерии
...
def save_to_target_sheet(participant_data, full_name):
    ...
    try:
        # --- СОХРАНЕНИЕ В ОСНОВНОЙ ЛИСТ (все данные) ---
        try:
            ...
            st.success(f"✅ Данные сохранены в лист '{target_sheet_name}'")
            
        except Exception as e:
            st.error(f"❌ Ошибка сохранения в основной лист: {e}")
            success = False

        # --- СОХРАНЕНИЕ В БУХГАЛТЕРСКИЙ ЛИСТ (только финансовые данные) ---
        try:
            # Пытаемся открыть бухгалтерский лист, если нет - создаем
            try:
                ...
                st.info(f"📊 Создан новый лист: '{ACCOUNTING_SHEET_NAME}'")
            ...
            st.success(f"✅ Данные сохранены в лист '{ACCOUNTING_SHEET_NAME}'")
...
if search_surname:
    ...
    if 'participant' in locals():
        ...
        if submitted:
            ...
            if save_to_target_sheet(data_to_save, full_name):
                st.success("✅ Данные успешно сохранены в оба листа!")
                # Очищаем кеш после сохранения
                st.cache_data.clear()
                st.balloons()  # Праздничный эффект :)
            else:
                st.error("❌ Не удалось сохранить данные в один из листов. Проверьте логи.")
