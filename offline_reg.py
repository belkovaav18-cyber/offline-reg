...
def optimize_image(image_path, max_size=(1920, 1080), quality=85):
    """
    Оптимизирует изображение перед конвертацией в base64
    """
    try:
...
def set_background(image_file):
    """
    Устанавливает фоновое изображение с оптимизацией
    """
    try:
        # Оптимизируем изображение
        img_data = optimize_image(image_file, max_size=(1920, 1080), quality=85)
        
        if img_data is None:
            # Если оптимизация не удалась, загружаем оригинал
        ...
        st.warning(f"Не удалось загрузить фоновое изображение: {e}")
        return False
...
try:
    ...
    set_background("Frame 11.png")  # Замените на путь к вашему изображению
    ...
    st.sidebar.warning("⚠️ Используется градиентный фон (файл не найден)")
    ...
    if 'participant' in locals():
        ...
        if submitted:
            ...
            if save_to_target_sheets(data_to_save, full_name):
                # Вместо шариков показываем волны
                st.balloons()  # Убираем шарики
                # Добавляем красивую анимацию волн при успехе
                success_waves = """
                <style>
                @keyframes wave {
                ...
                .wave {
                    ...
                    width: 200%;
                    height: 100%;
                    background: linear-gradient(180deg, rgba(64, 164, 223, 0.2) 0%, rgba(45, 140, 200, 0.4) 50%);
                    animation: wave 3s infinite linear;
                }
                .wave:nth-child(2) {
                    opacity: 0.5;
                    animation: wave 5s infinite linear;
                    background: linear-gradient(180deg, rgba(135, 206, 235, 0.3) 0%, rgba(173, 216, 230, 0.5) 100%);
                }
                </style>
                ...
                """
                components.html(success_waves, height=100)
